...
testParticipantInDescription();
    ...
    const meeting = meetings.find(m => m.id.meeting_id === 'bb3b6d13-0539-40ec-9108-0df4d30464ef');
    
    if (!meeting) {
      console.log('❌ Could not find the test meeting');
      return;
    }
    
    console.log('📅 Testing meeting:', meeting.id.meeting_id);
    
    // Call the exact same method that builds meeting properties
    const meetingData = await processor.buildMeetingProperties(meeting);
    
    console.log('\n📋 Generated meeting body:');
    console.log('==========================================');
    console.log(meetingData.properties.hs_meeting_body);
    console.log('==========================================\n');
    
    // Check if participants are in the description
    const meetingBody = meetingData.properties.hs_meeting_body;
    const hasParticipantSection = meetingBody.includes('Attio participants:');
    const hasTheoryVentures = meetingBody.includes('team@theory.ventures');
    const hasRJ = meetingBody.includes('rj@theoryvc.com');
    
    console.log('✅ Analysis:');
    console.log(`   Has "Attio participants:" section: ${hasParticipantSection}`);
    console.log(`   Includes team@theory.ventures: ${hasTheoryVentures}`);
    console.log(`   Includes rj@theoryvc.com: ${hasRJ}`);
    
    if (hasParticipantSection && hasTheoryVentures && hasRJ) {
      console.log('\n🎉 SUCCESS: All participants are included in the meeting description!');
    } else {
      console.log('\n❌ ISSUE: Participants are missing from the meeting description');
    }
