...
class VideoProcessor {
  ...
  async getVideoFiles() {
    try {
      console.log('🎥 Scanning videos directory for new files...');
      ...
      console.error('❌ Error scanning videos directory:', error.message);
      throw error;
    }
  }
  ...
  async checkExistingVideos(videoFiles) {
    console.log('🔍 Checking for videos already uploaded to HubSpot...');
    ...
    try {
      ...
      const newVideos = videoFiles.filter(video => {
        ...
        if (alreadyExists) {
          console.log(`⏭️  Skipping ${video.filename} - meeting ${video.meetingId} already has recording`);
        }
  ...
  async generateTranscript(videoFile) {
    console.log(`🎤 Generating transcript for ${videoFile.filename}...`);
...
[Actual transcription would appear here with speaker identification and timestamps]`;
      ...
      console.log(`✅ Transcript generated for ${videoFile.filename}`);
      return placeholderTranscript;
      
    } catch (error) {
      console.error(`❌ Error generating transcript for ${videoFile.filename}:`, error.message);
      throw error;
    }
  }
  ...
  async findMatchingMeeting(videoFile) {
    console.log(`🔍 Finding HubSpot meeting for video ${videoFile.filename}...`);
    ...
    try {
      ...
      if (matchedMeeting) {
        console.log(`✅ Found exact match: ${matchedMeeting.id} for video ${videoFile.filename}`);
        return {
      ...
      if (matchedMeeting) {
        console.log(`⚠️  Found partial match: ${matchedMeeting.id} for video ${videoFile.filename}`);
        return {
          meeting: matchedMeeting,
          confidence: 0.8,
      ...
      console.log(`❌ No matching meeting found for video ${videoFile.filename}`);
      return null;
      
    } catch (error) {
      console.error(`❌ Error finding meeting for ${videoFile.filename}:`, error.message);
      throw error;
    }
  }
  ...
  async uploadVideoToHubSpot(videoFile) {
    console.log(`📤 Uploading ${videoFile.filename} to HubSpot...`);
    ...
    try {
      ...
      console.log(`✅ Video uploaded: ${mockFileUrl}`);
      ...
      console.error(`❌ Error uploading ${videoFile.filename}:`, error.message);
      throw error;
    }
  }
  ...
  async updateMeetingWithVideo(meeting, videoFile, transcript, uploadedFile) {
    console.log(`📝 Updating HubSpot meeting ${meeting.id} with video and transcript...`);
...
${transcript}`;
      ...
      console.log(`✅ Meeting ${meeting.id} updated with video and transcript`);
  ...
  async createAssociations(hubspotMeetingId, videoFile) {
    console.log(`🔗 Creating associations for meeting ${hubspotMeetingId}...`);
    ...
    try {
      ...
      console.log(`ℹ️  Association creation skipped for ${hubspotMeetingId} - no attendee data available for new video`);
      
      return associationResults;
      
    } catch (error) {
      console.error(`❌ Error creating associations for meeting ${hubspotMeetingId}:`, error.message);
      throw error;
    }
  }
  ...
  async processVideo(videoFile) {
    console.log(`\n🎬 Processing video: ${videoFile.filename}`);
    ...
    try {
      ...
      result.success = true;
      console.log(`✅ Successfully processed ${videoFile.filename}`);
  ...
  async processAllVideos() {
    ...
    try {
      ...
      console.log(`\n📋 Processing ${videosToProcess.length} videos...\n`);
      ...
      console.log(`📁 Results saved to video_processing_results_${timestamp}.json`);
      
      return { processed: successful, errors: failed, results };
      
    } catch (error) {
      console.error('❌ Video processing pipeline failed:', error.message);
      throw error;
    }
  }
}
